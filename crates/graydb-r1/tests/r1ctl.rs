...
#[test]
fn legacy_planless_resume_archives_report_and_checksums_then_exits_nonzero() {
    let root = tempdir().unwrap();
    // A run created without set_plan reproduces pre-plan persistence state.
    let mut controller = graydb_r1::RunController::create(root.path(), "legacy").unwrap();
    controller.note("created before plan persistence").unwrap();
    drop(controller);
    let output = Command::new(env!("CARGO_BIN_EXE_r1ctl"))
        .args([
            "resume",
    ...
    assert!(state["stages"].get("seed").is_none());
}
